...
class RuntimeOptions(namedtuple('Runtime', 'executor replicas ports publicAddress')):
    required_options = []
    def __new__(cls, executor='docker', replicas=1, ports=[], publicAddress=[]):
        return super(RuntimeOptions, cls).__new__(cls, executor, replicas, ports, publicAddress)
...
class PackageOptions(namedtuple('Package', 'repository name builder publish verbose quiet py_version')):
    ...
    def __new__(cls, repository, name, builder='docker', publish=False, verbose=True, quiet=False, py_version=3):
